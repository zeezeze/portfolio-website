...
import { HUD_THEMES, HUD_THEME_IDS } from "../motion/themes";
import type { HudParams, HudThemeId } from "../motion/types";
import { PARAM_SLIDERS } from "./sliderConfig";

type ControlPanelProps = {
  params: HudParams;
  onChange: (next: HudParams) => void;
  onReset: () => void;
  visible: boolean;
  onToggle: () => void;
};
...
export function ControlPanel({
  ...
  const update = (patch: Partial<HudParams>) => {
  ...
  return (
    <>
      ...
      <aside
        ...
        aria-label="Milcore HUD controls"
      >
        <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
          <div>
            ...
            <h2 className="text-sm font-medium">Industrial HUD</h2>
          </div>
          <button
            type="button"
            onClick={onToggle}
        ...
        <div className="flex-1 space-y-5 overflow-y-auto px-4 py-4">
          <div>
            <p className="mb-2 text-xs text-white/50">Accent 主题</p>
              ...
              {HUD_THEME_IDS.map((id) => {
                ...
                return (
                  <button
                    key={id}
                    type="button"
                    onClick={() => update({ themeId: id as HudThemeId })}
